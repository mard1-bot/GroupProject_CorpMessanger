...
func (h *Handler) registerPublicKey(w http.ResponseWriter, r *http.Request) {
	...
	if req.PublicKey == "" {
		WriteErrorCode(w, http.StatusBadRequest, "missing_key", "Public key is required")
		return
	}
	...
	if err != nil {
		WriteErrorCode(w, http.StatusBadRequest, "invalid_base64", "Public key must be valid base64")
		return
	}

	// X25519 public key should be 32 bytes
	if len(decodedKey) != 32 {
		WriteErrorCode(w, http.StatusBadRequest, "invalid_key_length", "Public key must be 32 bytes (X25519)")
		return
	}
	...
	if req.PrivateKey != "" {
		...
		if err != nil {
			WriteErrorCode(w, http.StatusBadRequest, "invalid_base64", "Private key must be valid base64")
			return
		}
		// X25519 private key should be 32 bytes
		if len(decodedPrivateKey) != 32 {
			WriteErrorCode(w, http.StatusBadRequest, "invalid_key_length", "Private key must be 32 bytes (X25519)")
			return
		}
		...
		if err != nil {
			...
			WriteErrorCode(w, http.StatusInternalServerError, "internal", "Failed to encrypt private key")
			return
		}
	}
	...
	if err != nil {
		...
		WriteErrorCode(w, http.StatusInternalServerError, "internal", "Failed to check existing key")
		return
	}

	var key *models.EncryptionKey
	if existingKey != nil {
		// Key rotation: archive old key and create new one with incremented version
		h.logger.Info("rotating encryption key", "user_id", claims.UserID, "old_version", existingKey.KeyVersion)

		newVersion, err := h.storage.RotateEncryptionKey(r.Context(), claims.UserID, req.PublicKey, encryptedPrivateKey, "user_initiated")
		if err != nil {
			h.logger.Error("failed to rotate encryption key", "error", err)
			WriteErrorCode(w, http.StatusInternalServerError, "internal", "Failed to rotate encryption key")
			return
		}
		...
		if err != nil {
			h.logger.Error("failed to get rotated key", "error", err)
			WriteErrorCode(w, http.StatusInternalServerError, "internal", "Failed to get rotated key")
			return
		}

		h.logger.Info("encryption key rotated successfully", "user_id", claims.UserID, "new_version", newVersion)
	} else {
		...
		if err := h.storage.SaveUserPublicKey(r.Context(), key); err != nil {
			...
			WriteErrorCode(w, http.StatusInternalServerError, "internal", "Failed to save public key")
			return
		}
	}
...
func (h *Handler) getMyPublicKey(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		...
		WriteErrorCode(w, http.StatusInternalServerError, "internal", "Failed to get public key")
		return
	}

	if key == nil {
		WriteErrorCode(w, http.StatusNotFound, "not_found", "Public key not found")
		return
	}
	...
	if key.PrivateKey != "" {
		...
		if err != nil {
			...
			WriteErrorCode(w, http.StatusInternalServerError, "internal", "Failed to decrypt private key")
			return
		}
	}
...
func (h *Handler) getUserPublicKey(w http.ResponseWriter, r *http.Request) {
	...
	if userIDStr == "" {
		WriteErrorCode(w, http.StatusBadRequest, "missing_param", "user_id parameter is required")
		return
	}

	// Validate query parameter length to prevent DoS
	if len(userIDStr) > 100 {
		WriteErrorCode(w, http.StatusBadRequest, "invalid_param", "user_id parameter is too long")
		return
	}
	...
	if err != nil {
		...
		WriteErrorCode(w, http.StatusInternalServerError, "internal", "Failed to verify access")
		return
	}

	if !hasCommonChat {
		WriteErrorCode(w, http.StatusForbidden, "access_denied", "You don't share any chats with this user")
		return
	}
	...
	if key == nil {
		WriteErrorCode(w, http.StatusNotFound, "not_found", "Public key not found for this user")
		return
	}
...
func (h *Handler) getChatPublicKeys(w http.ResponseWriter, r *http.Request) {
	...
	if chatIDStr == "" {
		WriteErrorCode(w, http.StatusBadRequest, "missing_param", "chat_id parameter is required")
		return
	}
	...
	if !isMember {
		WriteErrorCode(w, http.StatusForbidden, "access_denied", "You are not a member of this chat")
		return
	}
	...
	if err != nil {
		...
		WriteErrorCode(w, http.StatusInternalServerError, "internal", "Failed to get public keys")
		return
	}
